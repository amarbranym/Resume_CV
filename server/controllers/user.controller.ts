...
export const forgetPassword = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const resetPasswordLink = `${process.env.RESET_PASSWORD_URL}/${resetToken}`;
      const data = { user: { name: user.name }, resetPasswordLink };
      const html = await ejs.renderFile(
        path.join(__dirname, "../mails/forgetPassword-mail.ejs"),
        data
      );
      try {
        await sendMail({
        ...
        res.status(201).json({
          success: true,
          message: `Please check your email: ${user.email} Reset password email sent successfully`,
        });
      } catch (error: any) {
        return next(new ErrorHandler(error.message, 400));
      }
