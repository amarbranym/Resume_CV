...
import { HobbieData } from "./FileldData";
import Button from '../ui/button/Button';
import { v4 as uuidv4 } from 'uuid';

const initialValues: { [key: string]: string } = {};
HobbieData.forEach((field) => {
    initialValues[field.name] = '';
});


const Hobbie: React.FC = () => {
    const [hobbie, setHobbie] = useState<any>([]);
    const [expanded, setExpanded] = useState<string | false>();
    console.log("education", hobbie)
    ...
    const addEducation = () => {
        ...
        setHobbie([...hobbie, { id: uniqueId, values: { ...initialValues } }]);
        setExpanded(uniqueId)
    };

    const handleFormSubmit = (values: any, id: string) => {

        setHobbie(hobbie.map((edu: any) => {
            if (edu.id === id) {
                return { ...edu, values };
            }
            return edu;
    ...
    const removeEducation = (id: string) => {
        setHobbie(hobbie.filter((edu: any) => edu.id !== id));
    };

    useEffect(() => {
        const id = uuidv4()
        setHobbie([...hobbie, { id: id, values: { ...initialValues } }]);
        setExpanded(id)
    }, [])
        ...
        <AccordionDetails>
            ...
            <div className='mt-4 text-center'>
                <Button type='button' bg='white' rounded='full' size='lg' onClick={addEducation}>
                    <AiOutlinePlus className='h-6 w-6' /> Hobbies
                </Button>
            </div>
        </AccordionDetails>
    );
